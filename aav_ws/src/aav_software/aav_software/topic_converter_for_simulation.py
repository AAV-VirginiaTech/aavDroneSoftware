...
class TopicConverter(Node):
    def __init__(self):
        ...
        self.new_position_subscriber = self.create_subscription(
            NewDronePosition, "/AAV/send_new_position", self.new_position_callback, 10
        )
        # Publisher(NewPosition): TC -> ArduPilot
        self.new_gps_publisher = self.create_publisher(
            GlobalPosition, "/ap/cmd_gps_pose", 10
        )

        self.set_mode = self.create_subscription(
            Mode, "/AAV/set_mode", self.set_mode_callback, 10
        )
    ...
    def new_position_callback(self, msg: NewDronePosition):
        ...
        should_publish, self.last_new_gps_publish_time = self.check_rate_limit(
            self.last_new_gps_publish_time
        )
        if should_publish:
